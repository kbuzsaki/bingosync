...
class Event(models.Model):
    ...
    def get_all_recent_for_room(room):
        ...
        for event_class in Event.event_classes():
            ...
            recent_events.extend(event_class.default_qs(room=room).filter(timestamp__gte=cutoff_dt))
...
class NewCardEvent(Event):
    ...
    def default_qs(cls, room, **kwargs):
        ...
        room_new_cards = NewCardEvent.objects.filter(player__room=room).values('player__room')
        latest_ts = room_new_cards.annotate(latest_ts=models.Max("timestamp")).values('latest_ts')
        return super(NewCardEvent, cls).default_qs(room, **kwargs).annotate(_latest_timestamp=models.Subquery(latest_ts))
